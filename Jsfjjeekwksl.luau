    ...
    Icons = (function()
      -- This file was @generated by Tarmac. It is not intended for manual editing.
        return {
            ...
            ["calendar"] = "rbxassetid://10709889505",
...
function redzlib:MakeWindow(Configs)
    ...
    function Window:MakeTab(paste, Configs)
        ...
        -- =============================================================
        -- ============== NOVA FUNCIONALIDADE DE AI CHAT ==============
        -- =============================================================
        function Tab:AddAIChat(Configs)
            local ChatName = Configs[1] or Configs.Name or Configs.Title or "AI Chat"
            local API_KEY = Configs.API_KEY or "AIzaSyDSIy5m7mTXlMMR_OOdCu2Af_EwoCd124w"
            local API_URL = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=" .. API_KEY
            
            -- Container principal do chat
            local ChatContainer = InsertTheme(Create("Frame", Container, {
                Size = UDim2.new(1, 0, 0, 300),
                BackgroundColor3 = Theme["Color Hub 2"],
                Name = "AIChatContainer"
            }), "Frame")
            Make("Corner", ChatContainer, UDim.new(0, 8))
            Make("Stroke", ChatContainer)
            
            -- Título do chat
            local TitleLabel = InsertTheme(Create("TextLabel", ChatContainer, {
            ...
            local ChatDisplay = InsertTheme(Create("ScrollingFrame", ChatContainer, {
            ...
            Make("Corner", ChatDisplay, UDim.new(0, 6))
            
            local ChatList = Create("UIListLayout", ChatDisplay, {
                SortOrder = Enum.SortOrder.LayoutOrder,
                Padding = UDim.new(0, 5)
            })
            
            local ChatPadding = Create("UIPadding", ChatDisplay, {
                PaddingTop = UDim.new(0, 5),
                PaddingBottom = UDim.new(0, 5),
            ...
            -- Área de input
            local InputContainer = InsertTheme(Create("Frame", ChatContainer, {
                Size = UDim2.new(1, -10, 0, 25),
                Position = UDim2.new(0, 5, 1, -5),
                AnchorPoint = Vector2.new(0, 1),
                BackgroundColor3 = Theme["Color Hub 1"]
            }), "Frame")
            Make("Corner", InputContainer, UDim.new(0, 6))
            Make("Stroke", InputContainer)
            
            local InputBox = InsertTheme(Create("TextBox", InputContainer, {
            ...
            local SendButton = InsertTheme(Create("TextButton", InputContainer, {
            ...
            local function AddMessage(message, isUser)
                local MessageFrame = Create("Frame", ChatDisplay, {
                    Size = UDim2.new(1, 0, 0, 0),
                    BackgroundTransparency = 1,
                    AutomaticSize = Enum.AutomaticSize.Y,
                    LayoutOrder = #ChatDisplay:GetChildren()
                })
                
                local MessageLabel = InsertTheme(Create("TextLabel", MessageFrame, {
                    Size = UDim2.new(1, 0, 0, 0),
                    AutomaticSize = Enum.AutomaticSize.Y,
                    BackgroundTransparency = 1,
                    Text = message,
                    TextSize = 12,
                    Font = Enum.Font.Gotham,
                    TextColor3 = isUser and Theme["Color Theme"] or Theme["Color Text"],
                    TextXAlignment = isUser and Enum.TextXAlignment.Right or Enum.TextXAlignment.Left,
                    TextWrapped = true,
                    RichText = true
                }), "Text")
                
                -- Atualizar CanvasSize e rolar para baixo
                task.wait()
                ChatDisplay.CanvasSize = UDim2.new(0, 0, 0, ChatList.AbsoluteContentSize.Y + 10)
            ...
            local function SendMessageToAI(message)
                ...
                local LoadingMessage = AddMessage("AI: Pensando...", false)
                ...
                task.spawn(function()
                    ...
                    if success then
                        ...
                        if successDecode and data.candidates and data.candidates[1] and data.candidates[1].content and data.candidates[1].content.parts and data.candidates[1].content.parts[1] then
                            aiResponse = data.candidates[1].content.parts[1].text
                        else
                            aiResponse = "Erro ao processar a resposta da API."
                        end
                    else
                        aiResponse = "Erro ao conectar com a API. Verifique sua chave e sua conexão."
                    end
                    
                    -- Atualizar a mensagem de carregamento com a resposta da IA
                    local label = LoadingMessage:FindFirstChildWhichIsA("TextLabel")
                    if label then
                        label.Text = "AI: " .. aiResponse
                    end

                    -- Atualizar CanvasSize e rolar para baixo novamente
                    task.wait()
                    ChatDisplay.CanvasSize = UDim2.new(0, 0, 0, ChatList.AbsoluteContentSize.Y + 10)
                    ChatDisplay.CanvasPosition = Vector2.new(0, ChatDisplay.CanvasSize.Y.Offset)
                end)
            ...
            AddMessage("AI: Olá! Eu sou seu assistente de IA. Como posso te ajudar hoje?", false)
            
            local AIChat = {}
            function AIChat:Visible(...) Funcs:ToggleVisible(ChatContainer, ...) end
            function AIChat:Destroy() ChatContainer:Destroy() end
            function AIChat:SetTitle(title)
                if title then
                    TitleLabel.Text = tostring(title)
                end
            end
            function AIChat:ClearChat()
                for _, child in pairs(ChatDisplay:GetChildren()) do
                    if child:IsA("Frame") then
                        child:Destroy()
                    end
                end
                AddMessage("AI: Chat limpo. Como posso te ajudar agora?", false)
            end
            
            return AIChat
        end
        -- =============================================================
        -- ============== FIM DA FUNCIONALIDADE DE AI CHAT ============
        -- =============================================================
